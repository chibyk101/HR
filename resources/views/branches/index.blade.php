<x-app-layout>
            ...
            <ul class="
              ...
              <li>
                <a class="
                  ...
                  text-gray-700
                  hover:bg-gray-100
                " href="#" onclick="showModal('create-modal')">Create Branch</a>
              </li>
            </ul>
          </div>
        </div>
  ...
  <div x-data="Model">
    <x-bladewind.modal size="big" title="Create branch" name="create-modal" show_action_buttons="false">
      <form @submit.prevent="handleCreate" action="{{ route('ajax.branches.store') }}" method="post" id="branch-create-form">
        @csrf
          ...
          <form>
            <div class="form-group mb-6">
              <input type="text" name="name" class="form-control block
                ...
                ease-in-out
                m-0
                focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" id="exampleInput7" placeholder="branch title">
            </div>
    ...
    <div x-show="!loading && branches.length && paginator.hasPages" class="flex justify-between mb-5">
      ...
      <div x-show="!loading && branches.length && paginator.hasPages"> showing
      ...
      <x-bladewind.table>
        ...
        <template x-for="branch in branches">
          <tr>
            ...
            <td x-text="branch.department?.name"></td>
          </td>
          <td>
              <div class="flex">
                <form @submit.prevent="handleDelete($event,branch.id)" class="mx-1">
                  @method('DELETE')
                  @csrf
                ...
                <x-bladewind.button color="blue" size="tiny" @click="openEdit(branch)">
    ...
    <div x-show="!loading && branches.length && paginator.hasPages" class="flex justify-center">
      <nav aria-label="Page navigation example">
        <ul class="flex list-style-none">
          <template x-for="link in paginator.links">
            <li :class="link.active ? 'active' : '' " :class="link.url ? '' : 'disabled'">
  ...
  <script>
    let Model = () => ({
      ...
      fetchData: function(url = "{{ route('ajax.branches.index') }}", query = null) {
        ...
        $.ajax({
          ...
          success: function(response) {
            model.paginator.next = response.branches.next_page_url
            model.paginator.prev = response.branches.prev_page_url
            model.paginator.links = response.branches.links
            model.paginator.from = response.branches.from
            model.paginator.to = response.branches.to
            model.paginator.total = response.branches.total
            model.branches = response.branches.data
            model.loading = false
            model.paginator.hasPages = response.branches.links.length > 3
            model.processing = false
          },
      ...
      openEdit(item) {
        let url = '{{ route("branches.edit", ":slug") }}';
        window.location.href = url.replace(':slug', item.id)
      },
      searchData(event) {
        this.fetchData("{{ route('ajax.branches.index') }}", $(event.target).val())
      }
    })
  </script>
  @endsection
