<x-app-layout>
  ...
  <x-bladewind::card>
    <form method="POST" class="signup-form" action="{{route('users.update',$user)}}" x-data="model" x-init="initModel" enctype="multipart/form-data">
      @csrf
      @method('PUT')

      <h1 class="my-2 text-2xl font-light text-blue-900/80">Edit Account</h1>
      ...
      <div class="grid gap-2 grid-cols-1 md:grid-cols-2">
        ...
        <div class="flex flex-col">
          <x-bladewind::input name="first_name" required="true" label="First Name" value="{{ old('first_name',$user->first_name) }}" />
          @error('first_name')
          <span class="text-red-600 my-1">{{ $message }}</span>
          @enderror
          <x-bladewind::input name="last_name" required="true" label="Last Name" value="{{ old('last_name',$user->last_name) }}" />
          @error('last_name')
          <span class="text-red-600 my-1">{{ $message }}</span>
          @enderror
          <x-bladewind::input name="middle_name" label="Other Names" value="{{ old('middle_name',$user->middle_name) }}" />
          @error('middle_name')
          <span class="text-red-600 my-1">{{ $message }}</span>
          @enderror
        </div>
        ...
        <div class="flex flex-col">
          <x-bladewind::input name="email" required="true" label="Email" value="{{ old('email',$user->email) }}" />
          @error('email')
          <span class="text-red-600 my-1">{{ $message }}</span>
          @enderror
        ...
        <div class="flex flex-col">
          ...
          <input type="date" name="dob" id="dob" placeholder="Date of birth" value="{{ old('dob',$user->dob?->toDateString()) }}" />
          @error('dob')
          <span class="text-red-600 my-1">{{ $message }}</span>
          @enderror
      ...
      <div class="grid gap-2 grid-cols-1 md:grid-cols-2">
        <div class="flex flex-col">
          ...
          <input type="date" name="company_doj" placeholder="Date of joining" value="{{ old('company_doj',$user->company_doj?->toDateString()) }}" />
          @php
          $levels = [
        ...
        <div class="flex flex-col">
          ...
          <select name="designation_id" class="px-4 py-2 my-1 bw-dropdown bg-white cursor-pointer text-left w-full text-gray-400 flex justify-between dark:text-white dark:border-slate-700 dark:bg-slate-600 dark:text-gray-300">
            <option value="">SELECT DESIGNATION</option>
            @if($user->designation)
            <option value="{{$user->designation_id}}" selected>{{ $user->designation->name }}</option>
            @endif
            <template x-for="designation in designations">
              <option x-bind:value="designation.id" x-text="designation.name"></option>
            </template>
          </select>
          ...
          <select name="officeType_id" class="px-4 py-2 my-1 bw-dropdown bg-white cursor-pointer text-left w-full text-gray-400 flex justify-between dark:text-white dark:border-slate-700 dark:bg-slate-600 dark:text-gray-300">
            ...
            @if($user->officeType)
            <option value="{{$user->officeType_id}}" selected>{{ $user->officeType->name }}</option>
            @endif
  ...
  <script>
    function model() {
      return {
        ...
        syncDepartments() {
          let list = [];
          this.user.departments.forEach(dept => {
            list.push(dept.id)
          });
          this.selected_departments = list
        }
      }
    }
  </script>
